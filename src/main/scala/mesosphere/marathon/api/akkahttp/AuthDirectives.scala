...
trait AuthDirectives extends AkkaDirectives {
  ...
  def authenticated(implicit authenticator: MarathonAuthenticator): Directive1[Identity] = extractRequest.flatMap { request =>
    extractClientIP.flatMap { clientIP =>
      val pluginRequest = HttpPluginFacade.request(request, clientIP)
  ...
  /**
    * This will authenticate the current request using the given authenticator as well as authorize using the
    * given authorizer based on the given action and resource
    *
    * Because this method takes implicits, you must explicitly call apply when applying the directive. IE:
    *
    *
    *     authenticatedAndAuthorized(action, resource).apply {
    *       ...
    *     }
    *
    ...
    * @param resource The entity for which authorization should be checked
    */
  def authenticatedAndAuthorized[Resource](action: AuthorizedAction[Resource], resource: Resource)(implicit
    authenticator: MarathonAuthenticator,
    authorizer: Authorizer,
    actionSet: AuthorizedActionSet[_ >: Resource]): Directive0 =
    authenticated.flatMap { implicit identity =>
      authorized(action, resource)
    }

  /**
    * This will authenticate the current request using the given authenticator as well as authorize using the
    * given authorizer based on the given resource.
    * The authorized action itself is derived from the http request method.
    *
    * Because this method takes implicits, you must explicitly call apply when applying the directive. IE:
    *
    *
    *     authenticatedAndAuthorized(resource).apply {
    *       ...
    *     }
    *
    * @param resource The entity for which authorization should be checked
    */
  def authenticatedAndAuthorized[Resource](resource: Resource)(implicit
    authenticator: MarathonAuthenticator,
    authorizer: Authorizer,
    actionSet: AuthorizedActionSet[_ >: Resource]): Directive0 =
    authenticated.flatMap { implicit identity =>
  ...
  implicit val groupAuthorizedActionSet = AuthorizedActionSet[Group](CreateGroup, UpdateGroup, DeleteGroup, ViewGroup)
  implicit val runSpecAuthorizedActionSet = AuthorizedActionSet[RunSpec](CreateRunSpec, UpdateRunSpec, DeleteRunSpec, ViewRunSpec)
  implicit val resourceAuthorizedActionSet = AuthorizedActionSet[AuthorizedResource](CreateResource, UpdateResource, DeleteResource, ViewResource)
}
