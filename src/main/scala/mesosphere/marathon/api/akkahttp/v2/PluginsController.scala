...
class PluginsController(
  ...
  /**
    ...
    */
  def plugins: Route = authenticatedAndAuthorized(Plugins).apply {
  ...
  /**
    ...
    */
  def pluginRoute(pluginId: String, path: String): Route = authenticatedAndAuthorized(Plugins).apply {
  ...
  override val route: Route = asLeader(electionService) {
    pathEnd {
      get { plugins }
    } ~
      path(ValidPluginId / Remaining) { pluginRoute }
  }
}
